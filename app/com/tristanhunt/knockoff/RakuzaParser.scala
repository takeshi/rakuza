...
class RakuzaParser extends RegexParsers with StringExtras {

  override def skipWhitespace = false

  def chunk: Parser[Chunk] = {
    endBlock | endBlock2 |simpleLink | simpleLink2| simpleBeginBlock | paramBeginBlock|
    simpleBeginBlock2 | paramBeginBlock2 | commandLine | horizontalRule | leadingStrongTextBlock | leadingEmTextBlock | bulletItem |
      numberedItem | indentedChunk | header | blockquote | linkDefinition |
      textBlockWithBreak | textBlock | emptyLines
  }

  val simpleLink: Parser[Chunk] =
    ("""\[""".r ~> """([^\]]*)""".r <~ """\][\t ]*""".r) ~
      ("""\(""".r ~> """<?([ \S&&[^)>]]*)>?""".r <~ """\)""".r) ^^ {
        case ~(value, url) => LinkChunk(value, url, None)
      }

  val simpleLink2: Parser[Chunk] =
    ("""\[""".r ~> """([^\]]*)""".r <~ """\][\t ]*""".r) ~
      (""":<?([ \S&&[^)>]]*)>?""".r <~ """\s*\n""".r) ^^ {
        case ~(value, url) => LinkChunk(value, url, None)
      }
  ...
  val begin = """\s*-+\s*begin\s+""".r
  val beginEnd = """\s*-+[\s]*\n""".r

  val begin2 = """\s*#+""".r
  val beginEnd2 = """[\s]*\n""".r

  def simpleBeginBlock: Parser[Chunk] =
    begin ~> simpleCommand <~ beginEnd ^^ { value => CommandChunk(value, "", true) }

  def simpleBeginBlock2: Parser[Chunk] =
    begin2 ~> simpleCommand <~ beginEnd2 ^^ { value => CommandChunk(value, "", true) }

  def paramBeginBlock: Parser[Chunk] =
    begin ~> paramCommand <~ beginEnd ^^ { case ~(name, param) => CommandChunk(name, param.trim(), true) }

  def paramBeginBlock2: Parser[Chunk] =
    begin2 ~> paramCommand <~ beginEnd2 ^^ { case ~(name, param) => CommandChunk(name, param.trim(), true) }

  def endBlock: Parser[Chunk] =
    """\s*(-+\s*end\s*-+)""".r ^^ { value => EndCommandChunk() }

  def endBlock2: Parser[Chunk] =
    """\s*#+end(\s+|$)""".r ^^ { value => EndCommandChunk() }

  def commandLine: Parser[Chunk] =
    (paramCommandLine | simpleCommandLine)

  val simpleCommand = """(\w|-)+""".r

  val paramCommand = simpleCommand ~ ("""((:|\s+)\w(\w|-)*(=\S+)?)+""".r)

  def simpleCommandLine: Parser[Chunk] =
    """\s*\$""".r ~> simpleCommand ^^ { value => CommandChunk(value, "", false) }

  def paramCommandLine: Parser[Chunk] =
    """\s*\$""".r ~> paramCommand ^^ {
      case ~(v, p) => CommandChunk(v, p, false)
    }
...
case class EndCommandChunk(content: String = "") extends Chunk {
  def appendNewBlock(list: ListBuffer[Block],
    remaining: List[(Chunk, Seq[Span], Position)],
    spans: Seq[Span], position: Position,
    discounter: Discounter) {
    //    list += Command(name, params, spans, position,Seq())
  }

}

case class LinkChunk(content: String, url: String, title: Option[String]) extends Chunk {

  def appendNewBlock(list: ListBuffer[Block],
    remaining: List[(Chunk, Seq[Span], Position)],
    spans: Seq[Span], position: Position,
    discounter: Discounter) {
    list += LinkBlock(spans, url, title, position)
  }

}

case class CommandChunk(name: String, params: String, nested: Boolean, content: String = "") extends Chunk {

  def appendNewBlock(list: ListBuffer[Block],
    remaining: List[(Chunk, Seq[Span], Position)],
    spans: Seq[Span], position: Position,
    discounter: Discounter) {
    list += Command(name, params, spans, position)
  }
...
class RakuzaSpanConverter(definitions: Seq[LinkDefinitionChunk])
  ...
  */
  ...
  private def hasMatchedClose(source: String, tag: String, from: Int,
    opens: Int): Option[(Int, CharSequence)] = {
