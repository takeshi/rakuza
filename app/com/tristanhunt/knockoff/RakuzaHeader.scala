...
case class EndCommand(position: Position) extends Block

case class LinkBlock(spans: Seq[Span], url: String, title: Option[String],position: Position)
  extends Block

case class Command(name: String, params: String, spans: Seq[Span], position: Position, children: ListBuffer[Block] = new ListBuffer[Block]) extends Block {

  lazy val paramMap = {
    LinkCommand.toParamMap(params)
  }

  override def toString(): String = {
    if (children.length == 0) {
      "Command(" + name + "," + paramMap + "," + position + ")"
    } else {
      var childrenString = new StringBuilder().append("\n")
      children.foreach { child =>
        childrenString.append(child.toString()).append("\n")
      }
      "Command(" + name + "," + paramMap + "," + position + "," + childrenString + ")"
    }
  }
